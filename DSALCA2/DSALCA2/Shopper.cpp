...
void Shopper::sortGames(list<Games*> &inGames)
{
	...
	while(!check)
	{
		cout << "Sort By" << endl;
		cout << "1. Title " << endl;
		cout << "2. Cost (Lowest to Highest)" << endl;
		cout << "3. Cost (Highest to Lowest)" << endl;
		cout << "4. Rating (Highest to Lowest) " << endl;
		cout << "5. Rating (Lowest to Highest) " << endl;
		cout << "Input: ";
		cin >> input;
			...
			if(input >=1 && input <=5)
			{
				...
				switch(input)
				{
					...
					case 2:	
							for(;GameItor!=GameEndItor;GameItor++)
							{
								inGames.sort(sortByCostLowToHigh);
							}
						break;
					case 3:	
							for(;GameItor!=GameEndItor;GameItor++)
							{
								inGames.sort(sortByCostHigh2Low);
							}
						break;
					case 4:	
							for(;GameItor!=GameEndItor;GameItor++)
							{
								inGames.sort(sortByRatingDesc);
							}
						break;
					case 5:	
							for(;GameItor!=GameEndItor;GameItor++)
							{
								inGames.sort(sortByRatingAsc);
							}
						break;
				} 	
			}
...
bool Shopper::sortByTitle(Games* lhs,Games* rhs)
{
	return lhs->getTitle()<rhs->getTitle();
}

bool Shopper::sortByCostLowToHigh(Games* lhs,Games* rhs)
{	
	return ((lhs->getCost())<(rhs->getCost()));
}

bool Shopper::sortByCostHigh2Low(Games* lhs,Games* rhs)
{
	return ((lhs->getCost())>(rhs->getCost()));
}

bool Shopper::sortByRatingAsc(Games* lhs,Games* rhs)
{
	return lhs->getAvgRating()<rhs->getAvgRating();
}

bool Shopper::sortByRatingDesc(Games* lhs,Games* rhs)
{
	return lhs->getAvgRating()>rhs->getAvgRating();
}